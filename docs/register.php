...
if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){
                    $username_err = "Ese nombre ya ha sido tomado.";
                } else{
    ...
    if(empty(trim($_POST["password"]))){
        $password_err = "Por favor ingresa un Password.";     
    } elseif(strlen(trim($_POST["password"])) < 6){
        $password_err = "Password debe tener al menos longitud 6.";
    } else{
    ...
    if(empty(trim($_POST["confirm_password"]))){
        ...
        if(empty($password_err) && ($password != $confirm_password)){
            $confirm_password_err = "El Password no es el mismo.";
        }
    }
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err)){
        ...
        //strip_tags(mysql_real_escape_string(trim($variable)));
        $param_username = strip_tags(trim($username));
